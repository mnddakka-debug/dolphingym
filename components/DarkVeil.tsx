...
void main(){
    ...
    // Map CPPN output intensity to deep-blue palette (navy → electric blue)
    float intensity = dot(col.rgb, vec3(0.299, 0.587, 0.114));
    col.rgb = mix(vec3(0.0, 0.01, 0.05), vec3(0.05, 0.22, 0.95), intensity);
...
function createShader(gl: WebGLRenderingContext, type: number, src: string) {
    ...
    if (!gl.getShaderParameter(s, gl.COMPILE_STATUS)) {
        console.warn('DarkVeil shader compile error:', gl.getShaderInfoLog(s));
        gl.deleteShader(s);
        return null;
    }
    return s;
...
function createProgram(gl: WebGLRenderingContext, vert: string, frag: string) {
    ...
    if (!gl.getProgramParameter(prog, gl.LINK_STATUS)) {
        console.warn('DarkVeil program link error:', gl.getProgramInfoLog(prog));
...
export default function DarkVeil({
    ...
    useEffect(() => {
        ...
        const resize = () => {
            const w = Math.floor(parent.clientWidth * resolutionScale);
            const h = Math.floor(parent.clientHeight * resolutionScale);
            canvas.width = w;
            canvas.height = h;
            gl.viewport(0, 0, w, h);
