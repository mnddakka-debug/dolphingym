    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh] text-center animate-in zoom-in duration-500">
        ...
        <button 
          ...
          className="w-full py-5 blue-bg rounded-[2rem] text-white font-black uppercase tracking-widest shadow-xl shadow-blue-500/20 active:scale-95 transition-all"
        >
          {t.saveChanges}
        </button>
      </div>
  ...
  return (
    <div className="flex flex-col gap-6 animate-in slide-in-from-bottom duration-500 pb-12">
      <div className="flex items-center justify-between">
        ...
        <button className="text-[10px] font-black uppercase tracking-widest text-blue-400 bg-blue-500/10 px-4 py-2 rounded-full border border-blue-500/20" onClick={() => setActiveTab('history')}>
          {t.history}
        </button>
      </div>
      ...
      <div className="flex flex-col gap-4">
        {MOCK_WORKOUTS.map((workout) => (
          <div 
            ...
            className="bg-[#0f0f0f] border border-white/5 rounded-[2.5rem] p-5 flex items-center gap-5 hover:border-blue-500/30 transition-all cursor-pointer group active:scale-[0.98] shadow-lg"
          >
            <div className="w-20 h-20 rounded-[1.5rem] bg-[#1a1a1a] flex items-center justify-center overflow-hidden relative border border-white/5 shadow-inner">
               <img 
               ...
               />
               <Zap size={24} className="absolute text-blue-400 opacity-50" />
            </div>
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                 <span className="text-[8px] font-black uppercase tracking-[0.2em] text-blue-500 bg-blue-500/10 px-2 py-0.5 rounded-md border border-blue-500/20">{workout.category}</span>
              </div>
              <h3 className="font-black text-xl uppercase tracking-tight">{workout.title}</h3>
              <div className="flex items-center gap-4 mt-2 text-[10px] font-bold text-gray-500 uppercase tracking-widest">
                <span className="flex items-center gap-1.5">
                  <Timer size={12} className="text-blue-400" /> {workout.duration} {t.mins}
                </span>
                <span className="flex items-center gap-1.5">
                  <Zap size={12} className="text-orange-400" /> {workout.calories} kcal
                </span>
              </div>
            </div>
            <div className="w-12 h-12 rounded-full blue-bg flex items-center justify-center shadow-xl group-hover:scale-110 transition-transform">
      ...
      {/* Featured Exercise Breakdown */}
      <div className="mt-4 bg-[#111111] rounded-[2.5rem] p-6 border border-white/5">
        ...
        <div className="space-y-4">
          {MOCK_WORKOUTS[0].exercises.map((ex) => (
            <div key={ex.id} className="p-4 rounded-3xl bg-white/5 border border-white/5 flex items-center justify-between hover:bg-white/10 transition-all">
              <div className="flex items-center gap-4">
                 ...
                 <div>
                    ...
                    <p className="text-[9px] text-gray-500 font-bold uppercase tracking-widest mt-0.5">{ex.sets} {t.sets} • {ex.reps} {t.reps}</p>
                 </div>
              </div>
      ...
      <button 
        ...
        className="w-full py-5 blue-bg rounded-[2rem] font-black text-white text-lg shadow-2xl shadow-blue-500/30 mt-4 active:scale-95 transition-all flex items-center justify-center gap-3 group uppercase tracking-widest"
      >
